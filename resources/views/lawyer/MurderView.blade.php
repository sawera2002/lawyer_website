                                    ...
                                    <div class="row">
                                        <div class="col-12">
                                            <table class="table">
                                                ...
                                                <tbody>
                                                    @foreach ($mur as $murder)
                                                    <tr>
                                                        <!-- database colum name -->
                                                        <td>{{$murder->id}}</td>
                                                        <td> <img src="/images/{{$murder->image}}" width="80px" alt=""> </td>
                                                        <td>{{$murder->name}}</td>
                                                        <td>{{$murder->law}}</td>
                                                        <td>{{$murder->paragraph}}</td>
                                                        <td><a href="{{route('murder.edit',$murder->id)}}" class="btn" id="upd">update</a></td>
                                                        <!-- delete work -->
                                                        <td>
                                                            <form action="{{route('murder.destroy',$murder->id)}}" method="post">
                                                                @csrf
                                                                @method('DELETE')
