                                            ...
                                            <table class="table">
                                                ...
                                                <tbody>
                                                    ...
                                                    <tr>
                                                        <!-- database colum name -->
                                                        <td>{{$poly->id}}</td>
                                                        <td> <img src="/images/{{$poly->image}}" width="80px" alt=""> </td>
                                                        <td>{{$poly->name}}</td>
                                                        <td>{{$poly->law}}</td>
                                                        <td>{{$poly->paragraph}}</td>
                                                        <td><a href="{{route('poly.edit',$poly->id)}}" class="btn" id="upd">update</a></td>
                                                        <!-- delete work -->
                                                        <td>
                                                            <form action="{{route('poly.destroy',$poly->id)}}" method="post">
                                                                @csrf
                                                                @method('DELETE')
